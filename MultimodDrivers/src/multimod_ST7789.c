...
void ST7789_SetWindow(int16_t x, int16_t y, int16_t w, int16_t h) {
    ...
    // ASSUMPTION: If past boundaries, adjust them here
    if(x < 0){
        x = 0;
    }
    if(xEnd > X_MAX){
    ...
    ST7789_WriteData((uint8_t)x);
    ST7789_WriteData((uint8_t)(xEnd >> 8));
    ST7789_WriteData((uint8_t)xEnd);

    // Set row address
    ST7789_WriteCommand(ST7789_RASET_ADDR);
    ST7789_WriteData((uint8_t)(y >> 8));
    ST7789_WriteData((uint8_t)y);
    ST7789_WriteData((uint8_t)(yEnd >> 8));
    ST7789_WriteData((uint8_t)yEnd);
