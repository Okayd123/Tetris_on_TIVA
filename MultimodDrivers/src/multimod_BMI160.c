...
void BMI160_Init() {
    ...
    // Magnetometer initialization (for the curious)
    SysCtlDelay(10000);
    BMI160_WriteRegister(BMI160_CMD_ADDR, 0x19);

    SysCtlDelay(10000);
    BMI160_WriteRegister(BMI160_IFCONF_ADDR, 0x20);

    SysCtlDelay(10000);
    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_CONFIG_O, 0x83);
    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_DEVICEADDR_O, 0x13 << 1);

    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_DATAADDR_O, 0x01);
    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_WRADDR_O, 0x4B);

    SysCtlDelay(5000);
    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_DATAADDR_O, 0x04);
    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_WRADDR_O, 0x51);

    SysCtlDelay(5000);
    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_DATAADDR_O, 0x0E);
    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_WRADDR_O, 0x52);

    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_DATAADDR_O, 0b00110000);
    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_WRADDR_O, 0x4C);

    SysCtlDelay(5000);
    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_RDADDR_O, 0x42);
    BMI160_WriteRegister(BMI160_MAGIF_O + MAGIF_CONFIG_O, 0b00000011);

    BMI160_WriteRegister(BMI160_MAGCONF_ADDR, 0x06);
    SysCtlDelay(5000);
...
void BMI160_WriteRegister(uint8_t addr, uint8_t data) {
    ...
    I2C_WriteMultiple(I2C_A_BASE, BMI160_ADDR, bytes, 2);
}
...
void BMI160_MagXYZGetResult(uint16_t* data) {
    ...
    BMI160_MultiReadRegister(BMI160_DATA_O + MAGX_O, bytes, 8);

    *(data++) = (bytes[1] << 8 | bytes[0]);
    *(data++) = (bytes[3] << 8 | bytes[2]);
    *(data++) = (bytes[5] << 8 | bytes[4]);
    *(data++) = (bytes[7] << 8 | bytes[6]);
